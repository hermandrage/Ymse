...
import numpy as np
from util import load, build_vocabulary, get_bags_of_sifts, make_avg_histogram, make_confusion_matrix
...
make_confusion_matrix(test_labels, pred_labels_knn, title='Normalised CM from Nearest neighbor ', categories=categories)


print('Using support vector machine to predict test set categories\n')
#TODO: YOU CODE svm_classify function from classifers.py
pred_labels_svm = svm_classify(train_image_feats, train_labels, test_image_feats, 45)

make_confusion_matrix(test_labels, pred_labels_svm, title='Normalised Confusion matrix from SVM', categories=categories)
